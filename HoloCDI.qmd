...
The concatenated and padded image $P$ is next put through a discrete, two-dimensional fast Fourier transform (FFT), the values of which are reduced to their absolute values (i.e., magnitudes), and squared, resulting in a matrix of values that is proportional to the expected intensity of sensor readings given the image, separator, reference, and padding,
\begin{equation}
V = \left| \, \textrm{fft2}(P) \, \right|^2.
\end{equation}
$P$ is of size $512 \times 1536,$ so the two-dimensional FFT requires 1536 one-dimensional FFTs of size 512 and 512 one-dimensional FFTs of size 1536.  @fig-fftout shows a visualization of $V$ after the FFT shift to center the low frequencies.

```{python}
#| fig-cap: Squared magnitudes of the FFT of the concatenated and padded image.  The plot is scaled to $\log (1 + V)$ in order to display the pattern.
#| label: fig-fftout
#| fig-align: center
#| fig-pos: 't'
...
Note that this has been shifted.  The result is a lovely tartan pattern with a highly concentrated beam in the low frequencies.


### Beamstop

To avoid the coherent X-ray beam from destroying the sensors with concentrated low-frequencies, the low frequency diffractions are blocked with a beamstop.  This blockage means zero photons arrive at sensors in the stopped positions.  The image must be reconstructed from only the remaining sensors.
...
```{python}
#| fig-cap: Illustration of beamstop after fftshift.  The solid blue rectangles occlude the low frequencies, so that photon flux is not measured at those positions.
#| label: fig-beamstop
#| fig-align: center
#| fig-pos: 't'
...
Let $N_p$ be the number of photons expected per sensor.  This is used like an exposure value in a Poisson model to set the scale of the expected photon counts to be 
\begin{equation}
...
Finally, the number of photons observed at each sensor in the grid is generated as a Poisson process, based on the rate $\lambda.$
\begin{equation}
Y_{i, j} \sim
\begin{cases}
...
Given $\lambda,$ a Poisson random number generator can be used to simulate data.  An example is shown in @fig-simy.  

```{python}
#| fig-cap: Sampled value of $Y$, which is distributed Poisson with rate $\lambda$.  The scaled values $\log(1 + Y)$ is shown.
#| label: fig-simy
#| fig-align: center
#| fig-pos: 't'
...
lamda = N_p / V.mean() * V
Y = sp.stats.poisson.rvs(lamda, random_state=1234)
log1pY = np.log1p(Y)
plt.figure(figsize=(9,3))
...
This completes the definition of the sampling distribution;  the likelihood is just the sampling distribution viewed as a function of the parameters for fixed data. 
