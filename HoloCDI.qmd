...
The first thing to do is to compile the Stan model.  This is done by transpiling from Stan to C++, then compiling the C+ and linking against the rest of Stan's math library and algorithms.  This is all handled by the following CmdStanPy call.
...
This estimate can be computed through optimization in Stan.
...
An alternative point estimation strategy can be derived by taking the point in the posterior with maximum posterior log density.

\begin{equation}
x^{**} = \textrm{arg max}_x \log p(x \mid y).
\end{equation}

Here, the Jacobian adjustment is included as part of the definition of $p(x \mid y)$.
