...
transformed data {
  ...
  matrix[d, N] separator = rep_matrix(0, d, N);             // seperator
  matrix[N, N + d] separator_R = append_col(separator, R);  // separator + ref
}
parameters {
...
model {
  X ~ icar(sigma);
  matrix[N, 2 * N + d] X0R = append_col(X, separator_R);
  matrix[M1, M2] Y = square(abs(pad_fft2(X0R, M1, M2)));
